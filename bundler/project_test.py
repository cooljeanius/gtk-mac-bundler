...
class Project_Test(unittest.TestCase):
    ...
    def test_d_get_prefix(self):
        ...
        self.failUnlessEqual(pfx, "/usr/local/gtk",
                             "Alternate prefix %s" % pfx)
...
# If get_name works, this will too.
    ...
    def test_h_evaluate_path(self):
        try:
            path = self.goodproject.evaluate_path(
        ...
        self.failUnlessEqual(path, os.path.join(name, os.path.join(
            "Contents/MacOS", self.goodproject.name)), "Bundle path evaluation failed %s" % path)
